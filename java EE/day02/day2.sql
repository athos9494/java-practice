...
-- 查询s_id为101和102的记录
	select count(s_id) from fruits where s_id = 101 or s_id = 102;       

-- 查询所有s_id不等于101也不等于102的记录
	select count(s_id) from fruits where s_id <> 101 and s_id != 102;       

-- 查询价格在2.00元到10.20元之间的水果名称和价格
	select f_price,f_name from fruits where f_price > 2 and f_price < 10.2;       

-- 查询价格在2.00元到10.20元之外的水果名称和价格
	select f_price,f_name from fruits where f_price < 2 or f_price > 10.2;       

-- 查找所有以’b’字母开头的水果
	select f_name from fruits where f_name like 'b%';       

-- 在fruits表中，查询f_name中包含字母’g’的记录
	select count(*) from fruits where f_name like '%g%';       
...
-- 在employee表中，查询dept_no等于10和20的所有记录。
	select count(*) from employee where dept_no in (10,20);
...
select *from employee where e_salary = (select max(e_salary) from employee where dept_no = 10);
select *from employee where e_salary = (select max(e_salary) from employee where dept_no = 30);
...
-- 在employee表中，查询到目前为止，工龄大于等于10年的员工信息。
	select * from employee where datediff('2020-06-23',hireDate)>3650;
...
-- 在fruits表中查询s_id = 101或者102，且f_price大于5，并且f_name=‘apple’的水果价格和名称
	select f_price,f_name from fruits where s_id = 101 and f_price > 5 and f_name = 'apple';
...
-- 查询fruits表的f_name字段值,并对其进行排序
	...
	select f_name from fruits order by char_length(f_name) asc;
    
-- 查询fruits表中的f_name和f_price字段，先按f_name排序，再按f_price排序
	select f_name,f_price from fruits order by char_length(f_name) desc,f_price desc ;
    
-- 查询fruits表中的f_name和f_price字段，对结果按f_price降序方式排序
	select f_name,f_price from fruits order by f_price desc;

-- 查询fruits表，先按f_price降序排序，再按f_name字段升序排序
	select *from fruits order by f_price desc,char_length(f_name) asc ;

-- 根据s_id对fruits表中的数据进行分组
	select * from fruits where s_id = 101;
	select * from fruits where s_id = 102;
	select * from fruits where s_id = 103;
	select * from fruits where s_id = 104;
	select * from fruits where s_id = 105;
	select * from fruits where s_id = 106;
	select * from fruits where s_id = 107;
...
-- 根据s_id对fruits表中的数据进行分组，并显示水果种类大于1的分组信息
	select s_id,count(*)from fruits where count(f_name)>1 group by s_id;
...
-- 将以上内容使用ORDER BY关键字按总订单价格排序显示结果
	select (quantity*item_price) from orderitems order by (quantity*item_price) asc;
-- 显示fruits表查询结果的前4行
