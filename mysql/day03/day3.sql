...
(
    ...
    -- constraint emp_dept_fk foreign key (deptId) references tb_dept
    
    
);
-- 修改tb_emp表，创建外键约束，使用tb_dept中的id作为外键，关联deptId。

alter table tb_emp add constraint emp_dept_fk foreign key (deptId) references tb_dept;
...
(
    ...
    deptId INT(11),
    salary FLOAT,
    constraint emp1_dept_fk foreign key (deptId) references tb_dept
...
alter table tb_emp drop foreign key emp1_dept_fk;
alter table tb_emp add constraint emp_dept1_fk foreign key (deptId) references tb_dept1;
...
select customers.c_id ,orders.c_id from customers left join orders on customers.c_id = orders.c_id;

-- 在customers表和orders表中，查询所有订单，包括没有客户的订单，右连接
select customers.c_id ,orders.c_id from customers right join orders on customers.c_id = orders.c_id;


-- 在customers表和orders表中，使用INNER JOIN语法查询customers表中ID为10001的客户的订单信息
select *from customers inner join orders on  customers.c_id = orders.c_id where customers.c_id = 10001;

-- 查询fruits表所有信息附加一列suppliers的供应商名称，使用INNER JOIN语法进行内连接查询，并对查询结果排序
select f_id,fruits.s_id,f_name,f_price,s_name from fruits inner join suppliers on fruits.s_id = suppliers.s_id;
...
select f_name as fruit_name, f_price as fruits_price from fruits as f1 where f_price < 8;  
...
select c_name,grade from score where stu_id = (select id from student where student.name = '李四');
...
-- 查询计算机成绩低于95的学生信息
select * from student where id = (select stu_id from score where grade < 95 and c_name = '计算机');

select stu_id from score where grade < 95 and c_name = '计算机';


-- 查询同时参加计算机和英语考试的学生的信息
select student.name,department,c_name,grade from student inner join score on student.id = score.stu_id where c_name = '计算机' and c_name = '英语';

-- 从student表和score表中查询出学生的学号，然后合并查询结果

-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
select student.name,department,c_name,grade from student left join score on student.id = score.stu_id;

-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩

select student.name,(2020 - birth),department,c_name,grade from student,score where address like '湖南%' group by c_name;
-- --------------------------------------------------------------
...
-- 取得每个部门最高薪水的人员名称
select ename,deptno,max(mgr) from emp group by deptno;

-- 哪些人的薪水在部门的平均薪水之上
select ename ,deptno,mgr ,avg(mgr)from emp where mgr > (select avg(mgr) from emp) group by deptno;
